...
<div class="space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 hover:text-amber-600 border border-slate-200 transition-all shadow-sm">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-slate-800">Light Intensity Analysis</h2>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h3 class="text-xl font-black text-slate-800 tracking-tight">Illuminance (Lux)</h3>
                            <span class="px-2 py-0.5 bg-green-100 text-[10px] font-bold text-green-600 rounded-lg uppercase tracking-wider">LIVE</span>
                        </div>
                        <p class="text-xs font-medium text-slate-400">Ambient light & solar radiation monitor</p>
                    </div>
                </div>

                <div class="text-right">
                    <div class="flex items-baseline justify-end gap-1.5">
                        <span class="text-4xl font-black text-slate-900 tabular-nums tracking-tighter" id="current-lux-large">{{ number_format($latest->lux ?? 0, 0) }}</span>
                        <span class="text-sm font-bold text-slate-400">lux</span>
                    </div>
        ...
        <div class="px-6 py-3 bg-slate-50/30 border-b border-slate-50 flex items-center justify-between overflow-x-auto gap-4 no-scrollbar">
            <div class="flex items-center gap-1 p-1 bg-white rounded-xl border border-slate-100 shadow-sm">
                ...
                <button class="timeframe-btn px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all active-timeframe bg-amber-50 text-amber-600 shadow-sm" data-range="1d">1D</button>
        ...
        <div class="relative p-2 md:p-4 bg-slate-50/50">
            <div id="lux-chart" class="w-full h-[400px]"></div>
    ...
    <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-6 transition-all">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div class="flex flex-col gap-1 w-full md:w-44">
                ...
                <input type="date" id="start-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all">
            </div>
            <div class="flex flex-col gap-1 w-full md:w-44">
                <label class="text-[9px] font-bold text-slate-400 uppercase ml-1">End Date</label>
                <input type="date" id="end-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500/20 transition-all">
            </div>
            <button id="apply-filter" class="w-full md:w-auto mt-4 bg-amber-600 hover:bg-amber-700 text-white px-8 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-amber-500/20 active:scale-95 flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-amber-200 transition-colors">
            ...
            <div class="flex items-baseline gap-1">
                <span class="text-3xl font-black text-slate-800" id="stat-max">{{ number_format($globalStats['max'], 0) }}</span>
                <span class="text-xs font-bold text-slate-400">lux</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-amber-200 transition-colors">
            ...
            <div class="flex items-baseline gap-1">
                <span class="text-3xl font-black text-slate-800" id="stat-avg">{{ number_format($globalStats['avg'], 0) }}</span>
                <span class="text-xs font-bold text-slate-400">lux</span>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const parseHistoryData = (data) => {
            ...
            return items
                .map(item => {
                    ...
                    const luxValue = item.lux !== undefined && item.lux !== null ? parseFloat(item.lux) : null;

                    return {
                        x: timestamp,
                        y: Number.isFinite(luxValue) ? luxValue : null
                    };
                })
                .filter(item => {
        ...
        const options = {
            ...
            annotations: {
                yaxis: [{
                    ...
                    label: {
                        ...
                        text: chartData.length > 0 ? Math.round(chartData[chartData.length - 1].y) + ' lux' : '0 lux'
                    }
                }],
                points: [{
            ...
            fill: {
                ...
                gradient: {
                    ...
                    stops: [0, 90, 100],
                    colorStops: [
                        { offset: 0, color: '#d97706', opacity: 0.4 },
            ...
            yaxis: {
                ...
                labels: {
                    style: { colors: '#94a3b8', fontSize: '11px', fontWeight: 600 },
                    formatter: (val) => Math.round(val) + ' lux'
                }
            },
            grid: {
                borderColor: '#f1f5f9',
                    ...
                    return `
                        <div class="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 shadow-2xl">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-3 h-3 rounded-full bg-amber-500 shadow-[0_0_10px_rgba(217,119,6,0.5)]"></div>
                                <div class="text-lg font-black tracking-tight">${Math.round(value)} <span class="text-xs font-bold text-slate-400">lux</span></div>
                            </div>
                        </div>
                    `;
                }
        ...
        const chart = new ApexCharts(document.querySelector("#lux-chart"), options);
        chart.render();
        ...
        const updateSummary = (globalData = null) => {
            ...
            if (globalData) {
                ...
                if (document.getElementById('stat-avg')) document.getElementById('stat-avg').innerText = Math.round(globalData.avg).toLocaleString('id-ID');
                if (document.getElementById('stat-count')) document.getElementById('stat-count').innerText = globalData.total.toLocaleString('id-ID');
            } else {
                const max = values.length > 0 ? Math.max(...values) : 0;
                const avg = values.length > 0 ? (values.reduce((a, b) => a + b, 0) / values.length) : 0;
                
                if (document.getElementById('stat-max')) document.getElementById('stat-max').innerText = Math.round(max).toLocaleString('id-ID');
                if (document.getElementById('stat-avg')) document.getElementById('stat-avg').innerText = Math.round(avg).toLocaleString('id-ID');
                if (document.getElementById('stat-count')) document.getElementById('stat-count').innerText = chartData.length.toLocaleString('id-ID');
            }
        };
                ...
                annotations: {
                    yaxis: [{
                        ...
                        label: {
                            ...
                            text: lastPoint ? Math.round(lastPoint.y) + ' lux' : '0 lux'
                        }
                    }],
                    points: lastPoint ? [{
                        x: lastPoint.x,
        ...
        const fetchTimeframeData = async (range, page = 1) => {
            try {
                const chartContainer = document.querySelector("#lux-chart");
                chartContainer.style.opacity = '0.5';
                
                const response = await fetch(`{{ route('monitoring.lux.history') }}?range=${range}&page=${page}`);
                ...
                document.querySelector("#lux-chart").style.opacity = '1';
            }
        };
        ...
        const fetchCustomRange = () => {
            ...
            document.querySelectorAll('.timeframe-btn').forEach(b => {
                b.classList.remove('bg-amber-50', 'text-amber-600', 'shadow-sm');
            ...
            currentPage = 1;
            updatePaginationUI();
            const url = `{{ route('monitoring.lux.history') }}?range=${range}&start_date=${start}&end_date=${end}`;
        ...
        document.querySelectorAll('.timeframe-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                btn.classList.add('bg-amber-50', 'text-amber-600', 'shadow-sm');
        ...
        if (window.Echo) {
            window.Echo.channel('sensor-data')
                .listen('.new-data', (e) => {
                    const data = e.data;
                    const luxValue = data.lux !== undefined && data.lux !== null ? parseFloat(data.lux) : null;
                    ...
                    if (!chartData.some(d => d.x === timestamp)) {
                        chartData.push({ x: timestamp, y: Number.isFinite(luxValue) ? luxValue : null });
                        chartData.sort((a, b) => a.x - b.x);
                        if (chartData.length > 1000) chartData.shift();
                        refreshChart();
                    }

                    const currentEl = document.getElementById('current-lux-large');
                    const trendEl = document.getElementById('trend-indicator');
                    
                    if (currentEl) {
                        const prevValue = parseFloat(currentEl.innerText.replace(/\./g, ''));
                        const newValue = Number.isFinite(luxValue) ? luxValue : 0;
                        currentEl.innerText = Math.round(newValue).toLocaleString('id-ID');
                        ...
                        if (trendEl) {
                            if (newValue > prevValue) {
                                ...
                                trendEl.className = 'flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-50 text-amber-600';
                            }
                            lucide.createIcons();
                        }
                    }
