...
<div class="space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-slate-800">Water Level Analysis</h2>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h3 class="text-xl font-black text-slate-800 tracking-tight">Water Level</h3>
                            <span class="px-2 py-0.5 bg-green-100 text-[10px] font-bold text-green-600 rounded-lg uppercase tracking-wider">LIVE</span>
                        </div>
                        <p class="text-xs font-medium text-slate-400">Flood risk & surface water monitoring</p>
                    </div>
                </div>
        ...
        <div class="relative p-2 md:p-4 bg-slate-50/50">
            <div id="water-level-chart" class="w-full h-[400px]"></div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-cyan-200 transition-colors">
            <div class="flex items-center gap-3 mb-4">
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Max Level</p>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const parseHistoryData = (data) => {
            ...
            return items
                .map(item => {
                    ...
                    const waterLevelValue = item.water_level !== undefined && item.water_level !== null ? parseFloat(item.water_level) : null;

                    return {
                        x: timestamp,
                        y: Number.isFinite(waterLevelValue) ? waterLevelValue : null
                    };
                })
                .filter(item => {
        ...
        const chart = new ApexCharts(document.querySelector("#water-level-chart"), options);
        chart.render();
        ...
        const updateSummary = (globalData = null) => {
            ...
            const largeDisplay = document.getElementById('current-water-level-large');
        ...
        const fetchTimeframeData = async (range) => {
            try {
                ...
                document.querySelector("#water-level-chart").style.opacity = '1';
            }
        };
        ...
        if (window.Echo) {
            window.Echo.channel('sensor-data')
                .listen('.new-data', (e) => {
                    const data = e.data;
                    const waterLevelValue = data.water_level !== undefined && data.water_level !== null ? parseFloat(data.water_level) : null;
                    ...
                    if (!chartData.some(d => d.x === timestamp)) {
                        chartData.push({ x: timestamp, y: Number.isFinite(waterLevelValue) ? waterLevelValue : null });
                        chartData.sort((a, b) => a.x - b.x);
                        if (chartData.length > 1000) chartData.shift();
                        refreshChart();
                    }

                    const currentEl = document.getElementById('current-water-level-large');
                    const trendEl = document.getElementById('trend-indicator');
                    
                    if (currentEl) {
                        const prevValue = parseFloat(currentEl.innerText);
                        const newValue = Number.isFinite(waterLevelValue) ? waterLevelValue : 0;
