...
<div class="space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 hover:text-indigo-600 border border-slate-200 transition-all shadow-sm">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-slate-800">Battery Analysis</h2>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Storage Performance Sync</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all">
        ...
        <div class="p-6 md:p-8 border-b border-slate-50">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <!-- Left: Title & Icon -->
                <div class="flex items-center gap-4">
                    <div class="flex-shrink-0 w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center">
                        <i data-lucide="battery" class="w-6 h-6"></i>
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            <h3 class="text-lg md:text-xl font-black text-slate-800 tracking-tight leading-tight">Battery Status</h3>
                            <span class="px-2 py-0.5 bg-green-100 text-[10px] font-bold text-green-600 rounded-lg uppercase tracking-wider">LIVE</span>
                        </div>
                        <p class="text-xs font-medium text-slate-400">Voltage and Current storage metrics</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between lg:justify-end gap-4 md:gap-8 bg-slate-50/50 lg:bg-transparent p-4 md:p-6 lg:p-0 rounded-[1.5rem] lg:rounded-none">
                    <div class="text-right lg:border-r border-slate-200 lg:pr-8">
                        ...
                        <div class="flex items-baseline justify-end gap-1.5">
                            <span class="text-2xl md:text-4xl font-black text-slate-900 tabular-nums tracking-tighter" id="current-voltage-large">{{ number_format($latest->voltage_baterai ?? 0, 1) }}</span>
                    ...
                    <div class="text-right">
                        ...
                        <div class="flex items-baseline justify-end gap-1.5">
                            <span class="text-2xl md:text-4xl font-black text-slate-900 tabular-nums tracking-tighter" id="current-current-large">{{ number_format($latest->current_baterai ?? 0, 2) }}</span>
        ...
        <div class="px-6 py-3 bg-slate-50/30 border-b border-slate-50 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-1 p-1 bg-white rounded-xl border border-slate-100 shadow-sm overflow-x-auto no-scrollbar w-full md:w-auto">
                ...
                <button class="timeframe-btn flex-1 md:flex-none px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all active-timeframe bg-indigo-50 text-indigo-600 shadow-sm" data-range="1d">1D</button>
            ...
            <div class="flex items-center justify-center gap-6 w-full md:w-auto px-2">
                ...
                <div class="flex items-center gap-2">
                    <div class="w-2.5 h-2.5 rounded-full bg-violet-500 shadow-[0_0_8px_rgba(139,92,246,0.4)]"></div>
        ...
        <div class="relative p-2 md:p-4 bg-slate-50/50">
            <div id="battery-chart" class="w-full h-[400px]"></div>
            ...
            @if($history->isEmpty())
                <div id="chart-placeholder" class="absolute inset-0 flex flex-col items-center justify-center bg-white/50 backdrop-blur-sm z-10">
                    ...
                    <p class="text-sm font-bold text-slate-500">Waiting for incoming battery data...</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-6 transition-all">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div class="flex flex-col gap-1 w-full md:w-44">
                ...
                <input type="date" id="start-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all">
            </div>
            <div class="flex flex-col gap-1 w-full md:w-44">
                <label class="text-[9px] font-bold text-slate-400 uppercase ml-1">End Date</label>
                <input type="date" id="end-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all">
            </div>
            <button id="apply-filter" class="w-full md:w-auto mt-4 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95 flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="bg-white p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-violet-200 transition-colors">
            <div class="flex items-center gap-3 mb-3 md:mb-4">
                <div class="w-7 h-7 md:w-8 md:h-8 bg-violet-50 text-violet-600 rounded-lg flex items-center justify-center">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        let voltageData = parseHistoryData(initialHistory, 'voltage_baterai');
        let currentData = parseHistoryData(initialHistory, 'current_baterai');
        ...
        const options = {
            ...
            yaxis: [
                ...
                },
                {
                    opposite: true,
                    seriesName: 'Current',
                    title: { text: 'Current (A)', style: { color: '#8b5cf6', fontWeight: 800 } },
                    labels: {
                        style: { colors: '#8b5cf6', fontWeight: 600 },
        ...
        const chart = new ApexCharts(document.querySelector("#battery-chart"), options);
        chart.render();
        ...
        const fetchTimeframeData = async (range) => {
            try {
                document.querySelector("#battery-chart").style.opacity = '0.5';
                const response = await fetch(`{{ route('monitoring.battery.history') }}?range=${range}`);
                const result = await response.json();
                
                if (result.success) {
                    voltageData = parseHistoryData(result.data, 'voltage_baterai');
                    currentData = parseHistoryData(result.data, 'current_baterai');
                    
                    const lastPoint = voltageData.length > 0 ? voltageData[voltageData.length - 1] : null;
                    const maxDate = lastPoint ? lastPoint.x : Date.now();
                    ...
                    if (range === 'custom') minDate = new Date(result.startTime).getTime();
                ...
                document.querySelector("#battery-chart").style.opacity = '1';
            }
        };
        ...
        const fetchCustomRange = () => {
            ...
            fetchTimeframeData(`custom&start_date=${start}&end_date=${end}`);
        };
            ...
            window.Echo.channel('sensor-data')
                .listen('.new-data', (e) => {
                    ...
                    if (!voltageData.some(d => d.x === timestamp)) {
                        voltageData.push({ x: timestamp, y: parseFloat(data.voltage_baterai) });
                        currentData.push({ x: timestamp, y: parseFloat(data.current_baterai) });
                        ...
                        currentGlobalStats.max_v = Math.max(currentGlobalStats.max_v, parseFloat(data.voltage_baterai));
                        currentGlobalStats.max_a = Math.max(currentGlobalStats.max_a, parseFloat(data.current_baterai));
                        updateSummary(currentGlobalStats);
                    }
                });
        }
