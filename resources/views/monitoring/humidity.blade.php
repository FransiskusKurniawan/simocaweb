...
<div class="space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 hover:text-cyan-600 border border-slate-200 transition-all shadow-sm">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <div>
                <h2 class="text-xl font-extrabold tracking-tight text-slate-800">Humidity Analysis</h2>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h3 class="text-xl font-black text-slate-800 tracking-tight">Air Humidity</h3>
                            <span class="px-2 py-0.5 bg-green-100 text-[10px] font-bold text-green-600 rounded-lg uppercase tracking-wider">LIVE</span>
                        </div>
                        <p class="text-xs font-medium text-slate-400">Atmospheric moisture monitoring</p>
                    </div>
                </div>

                <div class="text-right">
                    <div class="flex items-baseline justify-end gap-1.5">
                        <span class="text-4xl font-black text-slate-900 tabular-nums tracking-tighter" id="current-humidity-large">{{ number_format($latest->humidity ?? 0, 1) }}</span>
        ...
        <div class="px-6 py-3 bg-slate-50/30 border-b border-slate-50 flex items-center justify-between overflow-x-auto gap-4 no-scrollbar">
            <div class="flex items-center gap-1 p-1 bg-white rounded-xl border border-slate-100 shadow-sm">
                ...
                <button class="timeframe-btn px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all active-timeframe bg-cyan-50 text-cyan-600 shadow-sm" data-range="1d">1D</button>
        ...
        <div class="relative p-2 md:p-4 bg-slate-50/50">
            <div id="humidity-chart" class="w-full h-[400px]"></div>
    ...
    <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-center justify-between gap-6 transition-all">
        ...
        <div class="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
            <div class="flex flex-col gap-1 w-full md:w-44">
                ...
                <input type="date" id="start-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 transition-all">
            </div>
            <div class="flex flex-col gap-1 w-full md:w-44">
                <label class="text-[9px] font-bold text-slate-400 uppercase ml-1">End Date</label>
                <input type="date" id="end-date" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 transition-all">
            </div>
            <button id="apply-filter" class="w-full md:w-auto mt-4 bg-cyan-600 hover:bg-cyan-700 text-white px-8 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-cyan-500/20 active:scale-95 flex items-center justify-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-cyan-200 transition-colors">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-8 h-8 bg-cyan-50 text-cyan-600 rounded-lg flex items-center justify-center">
                    <i data-lucide="trending-up" class="w-4 h-4"></i>
                </div>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Max Humidity</p>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const parseHistoryData = (data) => {
            ...
            return items
                .map(item => {
                    ...
                    const humidityValue = item.humidity !== undefined && item.humidity !== null ? parseFloat(item.humidity) : null;

                    return {
                        x: timestamp,
                        y: Number.isFinite(humidityValue) ? humidityValue : null
                    };
                })
                .filter(item => {
        ...
        const options = {
            ...
            fill: {
                ...
                gradient: {
                    ...
                    stops: [0, 90, 100],
                    colorStops: [
                        { offset: 0, color: '#0891b2', opacity: 0.4 },
            ...
            yaxis: {
                ...
                min: function(min) {
                    ...
                    const values = chartData.map(d => d.y).filter(y => y !== null);
                    if (values.length === 0) return 0;
                    const dataMin = Math.min(...values);
                    const dataMax = Math.max(...values);
                    const diff = dataMax - dataMin;
                    if (diff < 10.0) {
                        return Math.max(0, Math.floor((dataMin - (10.0 - diff) / 2) * 10) / 10);
                    }
                    return Math.max(0, Math.floor((dataMin - diff * 0.1) * 10) / 10);
                },
                max: function(max) {
                    ...
                    const diff = dataMax - dataMin;
                    if (diff < 10.0) {
                        return Math.min(100, Math.ceil((dataMax + (10.0 - diff) / 2) * 10) / 10);
                    }
                    return Math.min(100, Math.ceil((dataMax + diff * 0.1) * 10) / 10);
                },
                labels: {
                    style: { colors: '#94a3b8', fontSize: '11px', fontWeight: 600 },
                    ...
                    return `
                        <div class="bg-slate-900 text-white p-4 rounded-2xl border border-slate-800 shadow-2xl">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-3 h-3 rounded-full bg-cyan-500 shadow-[0_0_10px_rgba(8,145,178,0.5)]"></div>
                                <div class="text-lg font-black tracking-tight">${value.toFixed(1)} <span class="text-xs font-bold text-slate-400">%</span></div>
                            </div>
                        </div>
                    `;
                }
        ...
        const updateSummary = (globalData = null) => {
            ...
            const largeDisplay = document.getElementById('current-humidity-large');
        ...
        const fetchTimeframeData = async (range, page = 1) => {
            try {
                ...
                const response = await fetch(`{{ route('monitoring.humidity.history') }}?range=${range}&page=${page}`);
                ...
                document.querySelector("#humidity-chart").style.opacity = '1';
            }
        };
        ...
        const fetchCustomRange = () => {
            ...
            document.querySelectorAll('.timeframe-btn').forEach(b => {
                b.classList.remove('bg-cyan-50', 'text-cyan-600', 'shadow-sm');
            ...
            currentPage = 1;
            updatePaginationUI();
            const url = `{{ route('monitoring.humidity.history') }}?range=${range}&start_date=${start}&end_date=${end}`;
        ...
        if (window.Echo) {
            window.Echo.channel('sensor-data')
                .listen('.new-data', (e) => {
                    const data = e.data;
                    const humidityValue = data.humidity !== undefined && data.humidity !== null ? parseFloat(data.humidity) : null;
                    ...
                    if (!chartData.some(d => d.x === timestamp)) {
                        chartData.push({ x: timestamp, y: Number.isFinite(humidityValue) ? humidityValue : null });
                        chartData.sort((a, b) => a.x - b.x);
                        if (chartData.length > 1000) chartData.shift();
                        refreshChart();
                    }

                    const currentEl = document.getElementById('current-humidity-large');
                    const trendEl = document.getElementById('trend-indicator');
                    
                    if (currentEl) {
                        const prevValue = parseFloat(currentEl.innerText);
                        const newValue = Number.isFinite(humidityValue) ? humidityValue : 0;
                        ...
                        if (trendEl) {
                            if (newValue > prevValue) {
                                ...
                                trendEl.className = 'flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-600';
                            }
                            lucide.createIcons();
                        }
                    }
