...
<div class="space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col transition-all">
        ...
        <div class="p-6 md:p-8 border-b border-slate-50">
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-center gap-3 min-w-0">
                    <div class="w-10 h-10 sm:w-12 sm:h-12 bg-orange-50 text-orange-600 rounded-2xl flex items-center justify-center shrink-0">
                        <i data-lucide="thermometer" class="w-5 h-5 sm:w-6 sm:h-6"></i>
                    </div>
                    <div class="min-w-0">
                        <div class="flex items-center gap-1.5 flex-wrap">
                            <h3 class="text-sm sm:text-xl font-black text-slate-800 tracking-tight truncate">Environmental Temperature</h3>
                        ...
                        <p class="text-[10px] sm:text-xs font-medium text-slate-400 truncate">Atmospheric thermal monitoring</p>
                    </div>
                </div>

                <div class="text-right shrink-0">
                    <div class="flex items-baseline justify-end gap-1 whitespace-nowrap">
                        <span class="text-2xl sm:text-4xl font-black text-slate-900 tabular-nums tracking-tighter" id="current-temperature-large">{{ $latest && $latest->temperature !== null ? number_format($latest->temperature, 1) : '0.0' }}</span>
                        <span class="text-xs sm:text-sm font-bold text-slate-400">°C</span>
                    </div>
        ...
        <div class="relative p-2 md:p-4 bg-slate-50/50">
            <div id="temperature-chart" class="w-full h-[300px] sm:h-[400px]"></div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const fetchTimeframeData = async (range, page = 1) => {
            try {
                ...
                const response = await fetch(`{{ route('monitoring.temperature.history') }}?range=${range}&page=${page}`);
